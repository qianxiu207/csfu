...
// =============================================================================
// 🔵 流量代理专用版 (Data Plane)
// ⚠️ 此代码仅处理 vless/ws 流量，无管理面板，无订阅功能
// =============================================================================

const UUID = ""; // 必须与主域名保持一致
...
export default {
  async fetch(r, env, ctx) {
    try {
      // 物理级防御：非 WebSocket 升级请求一律阻断（隐藏特征）
      ...
      if (remoteProxyIP) { try { finalProxyConfig = await parseIP(remoteProxyIP); } catch (e) {} }
      else if (url.pathname.includes('/proxyip=')) { try { const proxyParam = url.pathname.split('/proxyip=')[1].split('/')[0]; finalProxyConfig = await parseIP(proxyParam); } catch (e) {} } 
      else if (_PROXY_IP) { try { finalProxyConfig = await parseIP(_PROXY_IP); } catch (e) {} }
